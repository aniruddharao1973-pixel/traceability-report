...
//       const resp = await fetch(
//         `${API_BASE}/api/trace/analysis?uid=${productUid}`,
//       );
...
const AnimatedBackground = () => (
  <div className="fixed inset-0 -z-10 overflow-hidden">
    ...
    <motion.div
      animate={{ scale: [1, 1.2, 1], opacity: [0.4, 0.6, 0.4] }}
      transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
      className="absolute top-0 -left-48 w-48 sm:w-72 md:w-80 lg:w-96 h-48 sm:h-72 md:h-80 lg:h-96 bg-blue-300/40 rounded-full blur-3xl"
    />
    <motion.div
      animate={{ scale: [1, 1.3, 1], opacity: [0.3, 0.5, 0.3] }}
      transition={{
      ...
      className="absolute bottom-0 -right-48 w-48 sm:w-72 md:w-80 lg:w-96 h-48 sm:h-72 md:h-80 lg:h-96 bg-purple-300/30 rounded-full blur-3xl"
    />
    <motion.div
      animate={{ scale: [1, 1.1, 1], opacity: [0.3, 0.4, 0.3] }}
      transition={{
      ...
      className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 sm:w-72 md:w-80 lg:w-96 h-48 sm:h-72 md:h-80 lg:h-96 bg-indigo-300/30 rounded-full blur-3xl"
    />
    <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.3)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.3)_1px,transparent_1px)] bg-[size:32px_32px] sm:bg-[size:48px_48px] md:bg-[size:64px_64px]" />
    <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(255,255,255,0.2),transparent)]" />
  </div>
);
...
const AnalysisDashboard = ({ uid: externalUid }) => {
  ...
  const statusPill = (status) => {
    ...
    const s = String(status).toUpperCase();
    const map = {
      PASS: "bg-emerald-600 text-white px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-semibold shadow-md sm:shadow-lg",
      FAIL: "bg-rose-600 text-white px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-semibold shadow-md sm:shadow-lg",
      REWORK:
        "bg-amber-500 text-white px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-semibold shadow-md sm:shadow-lg",
      "REWORK DONE":
        "bg-orange-500 text-white px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-semibold shadow-md sm:shadow-lg",
    };
    return (
      <span
        className={
          map[s] ||
          "bg-gray-400 text-white px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-semibold shadow-md sm:shadow-lg"
        }
      >
        {s}
      </span>
  ...
  const StatCard = ({
    ...
    const iconBgMap = {
      blue: "bg-gradient-to-br from-blue-200/80 to-blue-100/60 text-blue-700 shadow-md sm:shadow-lg shadow-blue-400/30",
      emerald:
        "bg-gradient-to-br from-emerald-200/80 to-emerald-100/60 text-emerald-700 shadow-md sm:shadow-lg shadow-emerald-400/30",
      amber:
        "bg-gradient-to-br from-amber-200/80 to-amber-100/60 text-amber-700 shadow-md sm:shadow-lg shadow-amber-400/30",
      cyan: "bg-gradient-to-br from-cyan-200/80 to-cyan-100/60 text-cyan-700 shadow-md sm:shadow-lg shadow-cyan-400/30",
      purple:
        "bg-gradient-to-br from-purple-200/80 to-purple-100/60 text-purple-700 shadow-md sm:shadow-lg shadow-purple-400/30",
    };
    return (
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        whileHover={{ scale: 1.03, y: -6 }}
        className={`relative p-3 sm:p-4 md:p-5 lg:p-6 rounded-lg sm:rounded-xl md:rounded-2xl lg:rounded-3xl bg-gradient-to-br ${
          colorMap[color]
        } backdrop-blur-2xl border sm:border-2 ${
          glow
            ? "shadow-lg sm:shadow-xl md:shadow-2xl"
            : "shadow-md sm:shadow-lg md:shadow-xl"
        } hover:shadow-xl sm:hover:shadow-2xl transition-all duration-500`}
      >
        <div className="flex items-center justify-between">
          <div className="flex-1 pr-2 sm:pr-3">
            <p className="text-indigo-900 text-[9px] sm:text-[10px] md:text-xs font-extrabold uppercase tracking-wider mb-0.5 sm:mb-1 md:mb-2">
              {title}
            </p>
            <p className="text-lg sm:text-2xl md:text-3xl lg:text-4xl font-black text-gray-900 mb-0">
              {value}
            </p>
            {subtitle && (
          ...
          <div
            className={`p-1.5 sm:p-2 md:p-2.5 lg:p-3 rounded-lg sm:rounded-xl md:rounded-2xl ${iconBgMap[color]} backdrop-blur-sm`}
          >
  ...
  const StationNode = ({ station, isLast }) => {
    ...
    return (
      <div className="flex items-center shrink-0">
        <motion.div
          whileHover={{ scale: 1.03, y: -4 }}
          className={`relative p-2.5 sm:p-3 md:p-4 lg:p-5 rounded-lg sm:rounded-xl md:rounded-2xl backdrop-blur-2xl border sm:border-2 transition-all duration-300 min-w-[120px] sm:min-w-[150px] md:min-w-[180px] shadow-md sm:shadow-lg ${cardClass}`}
        >
          <div className="flex items-center justify-between mb-0.5 sm:mb-1 md:mb-2">
            <h4 className="font-bold text-gray-900 text-[10px] sm:text-xs md:text-sm">
              {station.station_name}
            </h4>
            {statusPill(station.status)}
          </div>
        ...
        {!isLast && (
          <div className="mx-1.5 sm:mx-2 md:mx-3">
            <motion.div
              ...
              animate={{ scaleX: 1 }}
              transition={{ duration: 0.45 }}
              className="w-6 sm:w-8 md:w-10 lg:w-12 h-1 sm:h-1.5 bg-gradient-to-r from-blue-500 via-cyan-500 to-blue-400 rounded-full shadow-md sm:shadow-lg"
            />
          </div>
        )}
      </div>
  ...
  // Loading / error / empty states - preserved with responsive sizes
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center p-3 sm:p-4 md:p-6 lg:p-12">
        <AnimatedBackground />
        <motion.div
          animate={{ rotate: 360 }}
          transition={{ duration: 1, repeat: Infinity, ease: "linear" }}
          className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 lg:w-16 lg:h-16 border-2 sm:border-3 md:border-4 border-blue-500 border-t-transparent rounded-full"
        />
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-3 sm:p-4 md:p-6 lg:p-12">
        ...
        <motion.div
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
          className="text-center p-3 sm:p-4 md:p-6 lg:p-8"
        >
          <AlertTriangle className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 lg:w-16 lg:h-16 text-rose-500 mx-auto mb-2 sm:mb-3 md:mb-4" />
  ...
  if (hasFetchedOnce && !analysisData) {
    return (
      <div className="min-h-screen flex items-center justify-center p-3 sm:p-4 md:p-6 lg:p-12">
        <AnimatedBackground />
        <div className="text-center">
          <Factory className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 lg:w-16 lg:h-16 text-blue-500 mx-auto mb-2 sm:mb-3 md:mb-4" />
          <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900 mb-1">
            No Data
          </h2>
          <p className="text-gray-600 text-xs sm:text-sm md:text-base">
  ...
  return (
    <>
      <style>{scrollbarStyles}</style>
      <div className="p-2 sm:p-3 md:p-4 lg:p-6 xl:p-12 relative">
        <AnimatedBackground />

        <div
          id="full-report-page"
          className="max-w-7xl mx-auto space-y-3 sm:space-y-4 md:space-y-6 lg:space-y-8 relative z-10"
        >
          {/* UID - responsive sizes */}
          <div className="flex justify-center">
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              className="
                inline-flex items-center gap-2 sm:gap-3 md:gap-4 
                px-3 py-1.5 sm:px-4 sm:py-2 md:px-5 md:py-2.5 lg:px-6 lg:py-3 xl:px-8 xl:py-3.5
                rounded-full 
                bg-white/20 
                border border-white/40 
                backdrop-blur-2xl 
            ...
            >
              <BarcodeIcon className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 text-orange-900 drop-shadow-sm" />
              <span className="text-indigo-800 text-xs sm:text-sm md:text-base lg:text-lg xl:text-xl font-extrabold tracking-wide">
                UID: {uid}
              </span>
            </motion.div>
          </div>
          ...
          <motion.div
            initial={{ opacity: 0, y: 12 }}
            animate={{ opacity: 1, y: 0 }}
            className="grid grid-cols-1 min-[480px]:grid-cols-2 lg:grid-cols-3 gap-2.5 sm:gap-3 md:gap-4 lg:gap-5 xl:gap-6"
          >
            <StatCard
          ...
          >
            ...
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              whileHover={{ scale: 1.03, y: -6 }}
              className="relative p-3 sm:p-4 md:p-5 lg:p-6 rounded-lg sm:rounded-xl md:rounded-2xl lg:rounded-3xl bg-gradient-to-br from-white/80 via-white/60 to-white/40 backdrop-blur-2xl border sm:border-2 border-white/50 shadow-md sm:shadow-lg md:shadow-xl hover:shadow-xl sm:hover:shadow-2xl transition-all duration-500 min-[480px]:col-span-2 lg:col-span-1"
            >
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <p className="text-blue-900 text-[9px] sm:text-[10px] md:text-xs font-extrabold uppercase tracking-wider mb-1 sm:mb-2">
                    Quality Summary
                  </p>
                  <div className="flex items-baseline space-x-3 sm:space-x-4 md:space-x-6">
                    <div>
                      <p className="text-lg sm:text-2xl md:text-3xl lg:text-4xl font-black text-emerald-600">
                        {passPercent}%
                      </p>
                      <p className="text-gray-900 text-[10px] sm:text-xs md:text-sm font-extrabold">
                        PASS
                      </p>
                    </div>
                    <div>
                      <p className="text-lg sm:text-2xl md:text-3xl lg:text-4xl font-black text-rose-600">
                        {failPercent}%
                      </p>
                      <p className="text-gray-900 text-[10px] sm:text-xs md:text-sm font-extrabold">
                        FAIL
                ...
                <div className="p-2 sm:p-2.5 md:p-3 lg:p-4 rounded-lg sm:rounded-xl md:rounded-2xl bg-gradient-to-br from-blue-200/80 to-blue-100/60 shadow-md sm:shadow-lg shadow-blue-400/30">
                  <PieChart className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-blue-700" />
                </div>
              </div>
            </motion.div>
          </motion.div>
          ...
          <motion.section
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.2 }}
            className="bg-white/70 rounded-lg sm:rounded-xl md:rounded-2xl lg:rounded-3xl p-3 sm:p-4 md:p-5 lg:p-6 xl:p-8 backdrop-blur-2xl border sm:border-2 border-white/50 shadow-lg sm:shadow-xl md:shadow-2xl"
          >
            <div className="flex items-center space-x-2 sm:space-x-3 mb-3 sm:mb-4 md:mb-5 lg:mb-6">
              <div className="p-1.5 sm:p-2 md:p-2.5 lg:p-3 rounded-lg sm:rounded-xl bg-gradient-to-br from-blue-200/80 to-blue-100/60 shadow-md sm:shadow-lg">
                <BarChart3 className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-blue-700" />
              </div>
              <h2 className="text-sm sm:text-base md:text-lg lg:text-xl xl:text-2xl font-bold text-gray-900">
                Production Flow
              </h2>
              <div className="flex-1 h-px bg-gradient-to-r from-blue-500/50 via-cyan-500/30 to-transparent ml-2 sm:ml-3"></div>
            </div>

            <div
              className="flex overflow-x-auto overflow-y-hidden pb-2 sm:pb-3 md:pb-4 space-x-2 sm:space-x-3 md:space-x-4 custom-scrollbar scroll-smooth"
              style={{
                WebkitOverflowScrolling: "touch",
                scrollBehavior: "smooth",
                overscrollBehavior: "contain",
          ...
          >
            <div className="flex items-center space-x-2 sm:space-x-3 mb-3 sm:mb-4 md:mb-5 lg:mb-6">
              <div className="p-1.5 sm:p-2 md:p-2.5 lg:p-3 rounded-lg sm:rounded-xl bg-gradient-to-br from-emerald-200/80 to-emerald-100/60 shadow-md sm:shadow-lg">
            ...
            {/* vertical list; responsive max heights */}
            <div className="max-h-[300px] sm:max-h-[380px] md:max-h-[450px] lg:max-h-[520px] xl:max-h-[600px] overflow-y-auto pr-1 sm:pr-2 custom-scrollbar">
              <div className="space-y-3 sm:space-y-4 md:space-y-5 lg:space-y-6">
                {measurement_data.every(
                  (m) => !m.measurements || m.measurements.length === 0,
                ) && (
                  <div className="p-2.5 sm:p-3 md:p-4 rounded-lg sm:rounded-xl bg-white/60 border border-gray-200 text-gray-700 text-xs sm:text-sm md:text-base text-center">
                    No measurement data recorded for PASS or FAIL stations
                  </div>
                )}
                ...
                {measurement_data
                  ...
                  .map((m, idx) => (
                    <motion.div
                      ...
                      transition={{ delay: idx * 0.04 }}
                      whileHover={{ scale: 1.01, y: -2 }}
                      className="p-2.5 sm:p-3 md:p-4 lg:p-5 xl:p-6 rounded-lg sm:rounded-xl md:rounded-2xl bg-gradient-to-br from-white/80 via-white/60 to-white/40 border sm:border-2 border-white/50 backdrop-blur-xl shadow-md sm:shadow-lg hover:shadow-lg sm:hover:shadow-xl transition-all duration-300"
                    >
                      <div className="flex items-center justify-between mb-2 sm:mb-3">
                        ...
                        <span
                          className={`px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs md:text-sm font-bold shadow-md ${
                            String(m.status).toUpperCase() === "PASS"
                              ? "bg-emerald-700 text-white"
                              : "bg-rose-600 text-white"
                          }`}
                      ...
                      m.measurements.length > 0 ? (
                        <div className="space-y-2 sm:space-y-3">
                          {m.measurements.map((meas, i) => (
                            <motion.div
                              ...
                              animate={{ opacity: 1, x: 0 }}
                              transition={{ delay: 0.06 + i * 0.03 }}
                              className="p-2 sm:p-3 md:p-4 rounded-lg sm:rounded-xl bg-gradient-to-br from-blue-50/60 to-purple-50/40 border border-blue-200/40 backdrop-blur-sm"
                            >
                              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 sm:gap-3">
                                <div className="flex-1 min-w-0">
                                  <div className="font-semibold text-gray-800 text-xs sm:text-sm md:text-base mb-0.5 sm:mb-1">
                                    {meas.testid || "Measurement"}
                                  </div>
                                  {meas.lsl && meas.hsl && (
                                    <div className="text-xs sm:text-sm text-gray-900 font-medium space-x-1 sm:space-x-2">
                                      <span className="inline-block px-1.5 sm:px-2 py-0.5 text-[10px] sm:text-xs md:text-sm rounded bg-blue-50/90">
                                        LSL: {meas.lsl}
                                      </span>
                                      <span className="inline-block px-1.5 sm:px-2 py-0.5 text-[10px] sm:text-xs md:text-sm rounded bg-purple-50/80">
                                        HSL: {meas.hsl}
                                      </span>
                                    </div>
                                  )}
                                ...
                                <div className="flex items-center gap-2 sm:gap-3">
                                  <div className="text-right">
                                    <div className="font-bold text-sm sm:text-base md:text-lg lg:text-xl text-gray-900">
                                      {meas.value}
                                    </div>
                                  ...
                                  <span
                                    className={`px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs md:text-sm font-semibold shadow-md ${
                                      String(meas.teststatus).toUpperCase() ===
                                      "PASS"
                                        ? "bg-emerald-600 text-white"
                                        : "bg-rose-600 text-white"
                        ...
                        <div className="text-xs sm:text-sm text-gray-600 italic p-2 sm:p-3 md:p-4 bg-gray-50/40 rounded-lg text-center">
                          No measurement data recorded for this station
                        </div>
                      )}
                    </motion.div>
