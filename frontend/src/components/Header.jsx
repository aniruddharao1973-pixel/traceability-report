...
//   // 🔎 DEBUG: dropdown state tracking
//   useEffect(() => {
//     console.log("STATE → isReportsOpen:", isReportsOpen);
//   }, [isReportsOpen]);

//   useEffect(() => {
//     console.log("STATE → isReworkReportsOpen:", isReworkReportsOpen);
//   }, [isReworkReportsOpen]);

//   useEffect(() => {
//     console.log("STATE → isSearchOpen:", isSearchOpen);
//   }, [isSearchOpen]);

//   // Outside click close
//   // Outside click close — FINAL FIX
...
//       const isReportsBtn = reportsBtnRef.current?.contains(e.target);
//       const isReworkBtn = reworkBtnRef.current?.contains(e.target);
//       const isSearchBtn = searchBtnRef.current?.contains(e.target);
//       const isProfileBtn = profileBtnRef.current?.contains(e.target);

//       // ✅ If click is on ANY toggle button, DON'T close anything
//       if (isReportsBtn || isReworkBtn || isSearchBtn || isProfileBtn) {
//         console.log("[DOCUMENT CLICK] on toggle button → ignore");
//         return;
//       }

//       // ✅ Check if click is inside any open panel
//       const isInsideReportsPanel = reportsRef.current?.contains(e.target);
//       const isInsideReworkPanel = reworkRef.current?.contains(e.target);
//       const isInsideSearchPanel = searchRef.current?.contains(e.target);
//       const isInsideProfilePanel = profileRef.current?.contains(e.target);
//       const isInsideHeader = headerRef.current?.contains(e.target);
//       const isInsideMobilePanels = mobilePanelsRef.current?.contains(e.target);

//       // If inside any panel or header, don't close
//       if (
//         isInsideReportsPanel ||
//         isInsideReworkPanel ||
//         isInsideSearchPanel ||
//         isInsideProfilePanel ||
//         isInsideHeader ||
//         isInsideMobilePanels
//       ) {
//         console.log("[DOCUMENT CLICK] inside panel/header → ignore");
//         return;
//       }

//       // Otherwise, close all panels
//       console.log("[DOCUMENT CLICK] truly outside → close all");
//       closeAll();
//     }
...
//     <header
//       ref={headerRef}
//       className="bg-gradient-to-r from-sky-400 to-purple-500 dark:from-purple-900 dark:to-indigo-900 shadow-lg transition-all duration-200"
//     >
//       <div className="container mx-auto px-2 sm:px-4 py-2 sm:py-2 relative">
//         {/* Mobile/Tablet Layout */}
//         <div ref={mobilePanelsRef} className="flex flex-col lg:hidden">
...
//                 ref={reportsBtnRef}
//                 onClick={(e) => {
//                   console.log("%c[REPORTS CLICK] fired", "color:green");
...
//                 ref={reworkBtnRef}
//                 onClick={() => {
//                   console.log("%c[REWORK CLICK] fired", "color:green");
//                   setIsReworkReportsOpen((prev) => !prev);
//                   setIsReportsOpen(false);
//                   setIsProfileOpen(false);
//                   setIsSearchOpen(false);
...
//                 ref={searchBtnRef}
//                 onClick={() => {
//                   console.log("%c[SEARCH CLICK] fired", "color:green");
...
// // ============================================
// // PANEL COMPONENTS - Defined OUTSIDE Header to prevent re-creation
// // ============================================
...
//       ${
//         isMobile
//           ? "fixed inset-x-4 top-24 sm:top-28 z-[100] max-h-[70vh]"
//           : "absolute right-0 mt-3 w-[420px] z-[80]"
//       }
//       glass-effect-strong dark:bg-gray-900/95 backdrop-blur-3xl rounded-2xl sm:rounded-3xl
//       shadow-2xl border border-white/30 dark:border-gray-700/50 overflow-hidden
//     `}
//     style={{
//       boxShadow:
//         "0 25px 50px -12px rgba(0, 0, 0, 0.25), 0 0 60px rgba(139, 92, 246, 0.3)",
...
//   >
//     {/* Header */}
//     <div className="relative bg-gradient-to-r from-sky-400 via-purple-500 to-indigo-600 dark:from-purple-900 dark:to-indigo-900 px-4 py-3 border-b border-white/20">
//       <div className="flex items-center justify-between">
//         <div className="flex items-center gap-3">
//           <div className="w-8 h-8 glass-effect backdrop-blur-xl rounded-xl flex items-center justify-center shadow-lg">
//             <svg
//               xmlns="http://www.w3.org/2000/svg"
...
//     >
//       {searchWarning && (
//         <div className="mb-3 text-xs text-amber-700 dark:text-amber-300 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 border border-amber-300 dark:border-amber-700 rounded-xl px-3 py-2 flex items-center gap-2">
//           <svg
...
//             type="text"
//             autoFocus={isMobile}
//             className="block w-full rounded-xl border-2 border-gray-300 dark:border-gray-600 shadow-lg focus:ring-0 focus:border-purple-500 text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white px-3 py-2.5 transition-all duration-300 placeholder:text-gray-400"
//             placeholder="Enter search value..."
//             value={searchValue}
//             onChange={(e) => setSearchValue(e.target.value)}
//             onKeyDown={(e) => e.key === "Enter" && handleSearch()}
...
//           <div className="relative">
//             <select
//               className="block w-full rounded-xl border-2 border-gray-300 dark:border-gray-600 shadow-lg focus:ring-0 focus:border-purple-500 text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white px-3 py-2.5 pr-10 transition-all duration-300 appearance-none font-medium"
//               value={searchType}
//               onChange={(e) => setSearchType(e.target.value)}
//             >
//               <option value="uid">UID</option>
...
//             type="button"
//             onClick={handleSearch}
//             className="flex-1 inline-flex items-center justify-center px-4 py-2.5 text-sm font-bold rounded-xl text-white bg-gradient-to-r from-sky-500 via-purple-600 to-indigo-600 hover:from-sky-600 hover:via-purple-700 hover:to-indigo-700 shadow-xl hover:scale-[1.02] transition-all duration-300"
//           >
//             <svg
...
//               if (onReset) onReset();
//             }}
//             className="inline-flex items-center justify-center px-4 py-2.5 text-sm font-bold rounded-xl text-gray-700 dark:text-gray-200 bg-gradient-to-r from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-600 border-2 border-gray-300 dark:border-gray-500 shadow-lg hover:scale-[1.02] transition-all duration-300"
//           >
//             <svg
//               className="w-4 h-4 mr-2"
//               fill="none"
...
// }) => (
//   <div
//     ref={!isMobile ? reportsRef : undefined}
//     className={`
//       ${
//         isMobile
//           ? "fixed inset-x-4 top-24 sm:top-28 z-[100] max-h-[70vh]"
//           : "absolute right-0 mt-2 w-[26rem] z-[50]"
//       }
//       bg-white dark:bg-gray-900 rounded-xl shadow-2xl p-4 sm:p-6
//       border border-purple-400/40 overflow-y-auto
//     `}
//     onClick={(e) => e.stopPropagation()} // Prevent clicks from bubbling
//   >
//     {/* Header */}
...
//   const searchBtnRef = useRef(null);
//   const reportsBtnRef = useRef(null);
//   const reworkBtnRef = useRef(null);
//   const reworkRef = useRef(null);

//   // Additional refs for mobile buttons
//   const mobileSearchBtnRef = useRef(null);
//   const mobileReportsBtnRef = useRef(null);
//   const mobileReworkBtnRef = useRef(null);
...
//   useEffect(() => {
//     function handleClickOutside(e) {
//       // Check if click is on any toggle button (desktop or mobile)
//       const isReportsBtn =
//         reportsBtnRef.current?.contains(e.target) ||
//         mobileReportsBtnRef.current?.contains(e.target);
//       const isReworkBtn =
//         reworkBtnRef.current?.contains(e.target) ||
//         mobileReworkBtnRef.current?.contains(e.target);
//       const isSearchBtn =
//         searchBtnRef.current?.contains(e.target) ||
...
//   // ============================================
//   // SHARED PANEL COMPONENTS (Used by both mobile and desktop)
//   // ============================================

//   // Search Panel Component
//   // const SearchPanel = ({ isMobile = false }) => (
//   //   <div
//   //     ref={!isMobile ? searchRef : undefined}
//   //     className={`
//   //       ${
//   //         isMobile
//   //           ? "fixed inset-x-4 top-24 sm:top-28 z-[100] max-h-[70vh]"
//   //           : "absolute right-0 mt-3 sm:mt-4 w-[440px] max-w-[95vw] z-[80]"
//   //       }
//   //       glass-effect-strong dark:bg-gray-900/95 backdrop-blur-3xl rounded-2xl sm:rounded-3xl
//   //       shadow-2xl border border-white/30 dark:border-gray-700/50 overflow-hidden
//   //     `}
...
//   // Reports Panel Component
//   // const ReportsPanel = ({ isMobile = false }) => (
//   //   <div
//   //     ref={!isMobile ? reportsRef : undefined}
//   //     className={`
//   //       ${
...
//               {/* Rework Button - Mobile */}
//               <button
//                 ref={mobileReworkBtnRef}
//                 onClick={(e) => {
//                   e.stopPropagation();
//                   toggleReworkDropdown();
//                 }}
//                 className="px-3 py-1.5 bg-white/20 hover:bg-white/30 backdrop-blur-sm rounded-lg border border-white/30 text-xs font-semibold text-white"
//               >
//                 Rework
...
//               {/* Desktop Reports Panel - position relative to button */}
//               {isReportsOpen && (
...
//             {/* Rework Button - Desktop */}
//             <div className="relative" ref={reworkRef}>
//               <button
//                 ref={reworkBtnRef}
//                 onClick={toggleReworkDropdown}
//                 className="group px-3 xl:px-5 py-2 xl:py-2.5 bg-white/20 hover:bg-white/30 backdrop-blur-sm rounded-xl border border-white/30 hover:border-white/50 transition-all duration-300 hover:scale-105 hover:shadow-xl hover:shadow-white/20 focus:outline-none focus:ring-2 focus:ring-white/50"
//                 title="Rework Reports"
//               >
//                 <span className="text-xs xl:text-sm font-semibold text-white">
...
//                 ref={searchBtnRef}
//                 onClick={toggleSearchDropdown}
//                 className="glow-button group relative w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 lg:w-12 lg:h-12 xl:w-14 xl:h-14 glass-effect hover:bg-white/30 rounded-xl sm:rounded-2xl border border-white/40 hover:border-white/60 transition-all duration-500 hover:scale-110 focus:outline-none focus:ring-2 sm:focus:ring-4 focus:ring-white/40 flex items-center justify-center overflow-hidden"
//                 title="Search"
//               >
//                 <svg
//                   xmlns="http://www.w3.org/2000/svg"
//                   className="h-5 w-5 xl:h-6 xl:w-6 text-white group-hover:rotate-90 group-hover:scale-110 transition-all duration-500"
//                   fill="none"
//                   viewBox="0 0 24 24"
//                   stroke="currentColor"
//                   strokeWidth={2.5}
...
//         {/* ============================================ */}
//         {/* MOBILE PANELS - Rendered outside layout divs */}
//         {/* ============================================ */}

//         {/* Mobile Search Panel */}
//         {isSearchOpen && window.matchMedia("(max-width: 1023px)").matches && (
//           <>
//             <div
...
//         {/* Mobile Reports Panel */}
//         {isReportsOpen && window.matchMedia("(max-width: 1023px)").matches && (
//           <>
//             <div
//               className="fixed inset-0 bg-black/50 z-[90]"
//               onClick={() => setIsReportsOpen(false)}
...
//               <div
//                 className="fixed inset-0 bg-black/50 z-[90]"
//                 onClick={() => setIsReworkReportsOpen(false)}
//               />
//               <div className="fixed inset-x-4 top-24 sm:top-28 z-[100]">
//                 <ReworkSelectorModal
//                   onClose={() => setIsReworkReportsOpen(false)}
//                   onGenerate={handleReworkGenerate}
//                   isMobile={true}
...
//                         ? "Rework Approved Report"
//                         : reportType ===
//                             "rework-approved-pending-from-production"
//                           ? "Rework Approved Pending From Production Report"
//                           : reportType === "rework-pending"
//                             ? "Rework Pending Report"
//                             : "Traceability Report"}
...
//         {/* ---------- Analytics 4.0 Child Window ---------- */}
//         {isAnalyticsOpen && (
//           <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-white/60 backdrop-blur-md p-2 sm:p-0">
//             <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-2xl w-full h-full sm:w-[98%] sm:h-[95%] lg:w-[95%] lg:h-[90%] overflow-hidden relative border border-blue-400/40 flex flex-col">
//               {/* Modal Header */}
//               <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-3 sm:px-5 py-2 sm:py-3 bg-gradient-to-r from-blue-50/90 via-purple-50/90 to-blue-50/90 backdrop-blur-xl border-b border-gray-200 gap-2">
//                 <div className="flex items-center gap-2 sm:gap-3">
...
//           />
//           <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4">
//             <div className="bg-gradient-to-br from-orange-600/90 to-orange-400/90 backdrop-blur-lg border border-orange-500/60 rounded-2xl sm:rounded-3xl shadow-2xl shadow-orange-700/30 w-full max-w-sm sm:max-w-md">
//               <div className="flex items-center justify-between p-4 sm:p-6 border-b border-white/10">
//                 <div className="flex items-center gap-2 sm:gap-3">
//                   <div className="w-8 sm:w-10 h-8 sm:h-10 bg-white/20 rounded-xl flex items-center justify-center">
//                     <Info className="w-4 sm:w-5 h-4 sm:h-5 text-white" />
...
// Search Panel Component - RESPONSIVE
const SearchPanel = ({
  isMobile,
  searchRef,
  searchValue,
  ...
  <div
    ...
    className={`
      ${
        isMobile
          ? "fixed inset-2 top-16 sm:inset-4 sm:top-20 md:top-24 z-[100] max-h-[80vh] sm:max-h-[75vh]"
          : "absolute right-0 mt-2 sm:mt-3 w-[320px] sm:w-[380px] md:w-[420px] z-[80]"
      }
      glass-effect-strong dark:bg-gray-900/95 backdrop-blur-3xl 
      rounded-xl sm:rounded-2xl md:rounded-3xl 
      shadow-xl sm:shadow-2xl border border-white/30 dark:border-gray-700/50 overflow-hidden
    `}
    style={{
      boxShadow:
        "0 15px 30px -10px rgba(0, 0, 0, 0.2), 0 0 40px rgba(139, 92, 246, 0.2)",
    }}
    onClick={(e) => e.stopPropagation()}
  >
    {/* Header */}
    <div className="relative bg-gradient-to-r from-sky-400 via-purple-500 to-indigo-600 dark:from-purple-900 dark:to-indigo-900 px-3 sm:px-4 py-2 sm:py-3 border-b border-white/20">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 glass-effect backdrop-blur-xl rounded-lg sm:rounded-xl flex items-center justify-center shadow-lg">
            <svg
              xmlns="http://www.w3.org/2000/svg"
    ...
    {/* Content */}
    <div className="p-3 sm:p-4 max-h-[50vh] sm:max-h-[55vh] md:max-h-[60vh] overflow-y-auto bg-white/95 dark:bg-gray-900/95">
      {searchWarning && (
        <div className="mb-2 sm:mb-3 text-[10px] sm:text-xs text-amber-700 dark:text-amber-300 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 border border-amber-300 dark:border-amber-700 rounded-lg sm:rounded-xl px-2 sm:px-3 py-1.5 sm:py-2 flex items-center gap-1.5 sm:gap-2">
          <svg
      ...
      <div className="space-y-2 sm:space-y-3">
        {/* Search input */}
        <div>
          <label className="block text-[10px] sm:text-xs font-bold text-gray-700 dark:text-gray-300 mb-1 sm:mb-1.5 flex items-center gap-1.5 sm:gap-2">
            <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-gradient-to-r from-sky-500 to-purple-600"></span>
            Find
          </label>
          <input
            type="text"
            autoFocus={isMobile}
            className="block w-full rounded-lg sm:rounded-xl border sm:border-2 border-gray-300 dark:border-gray-600 shadow-md sm:shadow-lg focus:ring-0 focus:border-purple-500 text-xs sm:text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white px-2.5 sm:px-3 py-2 sm:py-2.5 transition-all duration-300 placeholder:text-gray-400"
            placeholder="Enter search value..."
            value={searchValue}
            onChange={(e) => setSearchValue(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && handleSearch()}
        ...
        <div>
          ...
          <div className="relative">
            <select
              className="block w-full rounded-lg sm:rounded-xl border sm:border-2 border-gray-300 dark:border-gray-600 shadow-md sm:shadow-lg focus:ring-0 focus:border-purple-500 text-xs sm:text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white px-2.5 sm:px-3 py-2 sm:py-2.5 pr-8 sm:pr-10 transition-all duration-300 appearance-none font-medium"
              value={searchType}
              onChange={(e) => setSearchType(e.target.value)}
            >
              <option value="uid">UID</option>
        ...
        <div className="flex flex-col sm:flex-row gap-2 pt-1 sm:pt-2">
          <button
            type="button"
            onClick={handleSearch}
            className="flex-1 inline-flex items-center justify-center px-3 sm:px-4 py-2 sm:py-2.5 text-xs sm:text-sm font-bold rounded-lg sm:rounded-xl text-white bg-gradient-to-r from-sky-500 via-purple-600 to-indigo-600 hover:from-sky-600 hover:via-purple-700 hover:to-indigo-700 shadow-lg sm:shadow-xl hover:scale-[1.02] transition-all duration-300"
          >
            <svg
          ...
          <button
            ...
            className="inline-flex items-center justify-center px-3 sm:px-4 py-2 sm:py-2.5 text-xs sm:text-sm font-bold rounded-lg sm:rounded-xl text-gray-700 dark:text-gray-200 bg-gradient-to-r from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-600 border sm:border-2 border-gray-300 dark:border-gray-500 shadow-md sm:shadow-lg hover:scale-[1.02] transition-all duration-300"
          >
            <svg
              className="w-3 h-3 sm:w-4 sm:h-4 mr-1.5 sm:mr-2"
              fill="none"
...
// Reports Panel Component - RESPONSIVE
const ReportsPanel = ({
  isMobile,
  reportsRef,
  reportType,
  ...
  ${
    isMobile
      ? "fixed inset-x-2 top-14 z-[100] max-h-[70vh]"
      : "absolute right-0 mt-2 w-[280px] sm:w-[350px] md:w-[26rem] z-[50]"
  }
...
`}
  ...
  >
    ...
    <div className="flex items-center justify-between mb-3 sm:mb-4">
      <div className="flex items-center gap-2 sm:gap-3">
        <div className="p-1.5 sm:p-2 md:p-3 bg-gradient-to-br from-sky-400 to-purple-600 rounded-lg sm:rounded-xl shadow-lg">
          <BarChart3 className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-white" />
        </div>
        <h2 className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold bg-gradient-to-r from-sky-600 to-purple-600 bg-clip-text text-transparent">
          Reports Panel
        </h2>
      </div>
      {isMobile && (
        <button
          onClick={() => setIsReportsOpen(false)}
          className="p-1.5 sm:p-2 rounded-full bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700"
        >
    ...
    <button
      onClick={handleApply}
      disabled={isLoading}
      className="w-full flex items-center justify-center px-3 sm:px-4 md:px-6 py-2 sm:py-2.5 md:py-3 rounded-lg sm:rounded-xl font-bold text-xs sm:text-sm md:text-base text-white bg-gradient-to-r from-sky-500 via-purple-500 to-sky-600 hover:scale-[1.02] transition-all duration-300 disabled:opacity-70 disabled:cursor-not-allowed"
    >
      {isLoading ? (
        <div className="flex items-center">
          ...
          <span className="text-xs sm:text-sm">Generating Report...</span>
        </div>
      ) : (
        <>
...
export function Header({
  ...
  return (
    <header
      ...
      className="bg-gradient-to-r from-sky-400 to-purple-500 dark:from-purple-900 dark:to-indigo-900 shadow-lg transition-all duration-200"
    >
      <div className="container mx-auto px-2 sm:px-3 md:px-4 py-1.5 sm:py-2 relative">
        {/* Mobile/Tablet Layout */}
        <div ref={mobilePanelsRef} className="flex flex-col lg:hidden">
          {/* Top Row - Logo and Theme */}
          <div className="flex items-center justify-between mb-1.5 sm:mb-2">
            <h1 className="text-sm sm:text-base md:text-lg font-bold text-white">
              Micrologic
              <sup className="text-[8px] sm:text-[10px] md:text-xs">®</sup>
            </h1>
            <div className="flex items-center gap-1.5 sm:gap-2">
              <button
                onClick={toggleTheme}
                className="text-white hover:text-gray-100 p-1.5 sm:p-2"
              >
                {theme === "dark" ? (
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
              ...
              <div className="relative" ref={profileRef}>
                <button
                  ref={profileBtnRef}
                  onClick={toggleProfileDropdown}
                  className="h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8 rounded-full bg-white text-purple-600 flex items-center justify-center font-semibold text-xs sm:text-sm"
                >
                  <span>A</span>
                </button>
                {isProfileOpen && (
                  <div className="absolute right-0 mt-2 w-36 sm:w-40 md:w-48 bg-white dark:bg-gray-800 rounded-md shadow-lg py-1 border dark:border-gray-700 z-[9999]">
                    <a
                      href="#"
                      className="block px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700"
                    >
                      Your Profile
                    </a>
                    <a
                    ...
                    <a
                      href="#"
                      className="block px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 border-t dark:border-gray-700"
                    >
                      Sign out
                    </a>
                  </div>
          ...
          <div className="flex items-center justify-between gap-1.5 sm:gap-2 flex-wrap">
            <button
              ...
              className="flex-1 min-[360px]:flex-initial px-2 sm:px-3 py-1 sm:py-1.5 bg-white/20 hover:bg-white/30 backdrop-blur-sm rounded-md sm:rounded-lg border border-white/30 text-[10px] sm:text-xs font-semibold text-white"
            >
              Analytics
            </button>
            ...
            <div className="flex gap-1.5 sm:gap-2">
              ...
              <button
                ...
                className="px-2 sm:px-3 py-1 sm:py-1.5 bg-white/20 hover:bg-white/30 backdrop-blur-sm rounded-md sm:rounded-lg border border-white/30 text-[10px] sm:text-xs font-semibold text-white"
              >
                Reports
              </button>
              ...
              <button
                ...
                className="w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 glass-effect hover:bg-white/30 rounded-lg sm:rounded-xl border border-white/40 flex items-center justify-center"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
        ...
        <div className="hidden lg:flex items-center justify-between">
          <div className="flex items-center ml-2 xl:ml-6">
            ...
            <button
              ...
              className="ml-6 xl:ml-20 mr-3 xl:mr-8 group flex items-center gap-2 px-3 xl:px-5 py-1.5 xl:py-2.5 bg-white/20 hover:bg-white/30 backdrop-blur-sm rounded-lg xl:rounded-xl border border-white/30 hover:border-white/50 transition-all duration-300 hover:scale-105 hover:shadow-xl hover:shadow-white/20 focus:outline-none focus:ring-2 focus:ring-white/50"
              title="Open Industry 4.0 Analytics Dashboard"
            >
              <BarChart3 className="h-4 xl:h-5 w-4 xl:w-5 text-white group-hover:rotate-12 transition-transform duration-300" />
              <span className="text-xs xl:text-sm font-semibold text-white whitespace-nowrap">
          ...
          {/* Reports Dashboard Label - Desktop only */}
          <div className="absolute left-1/2 transform -translate-x-[75%] hidden xl:flex items-center gap-3">
            <div className="p-2 bg-white/25 backdrop-blur-sm rounded-lg border border-white/30 shadow-lg">
          ...
          <div className="flex items-center space-x-2 xl:space-x-4 mr-2 xl:mr-8 relative z-[60]">
            ...
            <div className="relative">
              <button
                ref={reportsBtnRef}
                onClick={toggleReportsDropdown}
                className="group flex items-center gap-2 px-3 xl:px-5 py-1.5 xl:py-2.5 bg-white/20 hover:bg-white/30 backdrop-blur-sm rounded-lg xl:rounded-xl border border-white/30 hover:border-white/50 transition-all duration-300 hover:scale-105 hover:shadow-xl hover:shadow-white/20 focus:outline-none focus:ring-2 focus:ring-white/50"
                title="Traceability & Pass Rate Report"
              >
                <BarChart3 className="h-4 xl:h-5 w-4 xl:w-5 text-white group-hover:rotate-12 transition-transform duration-300" />
                <span className="text-xs xl:text-sm font-semibold text-white whitespace-nowrap hidden xl:inline">
            ...
            <div className="relative" ref={reworkRef}>
              <button
                ref={reworkBtnRef}
                onClick={toggleReworkDropdown}
                className="group px-3 xl:px-5 py-1.5 xl:py-2.5 bg-white/20 hover:bg-white/30 backdrop-blur-sm rounded-lg xl:rounded-xl border border-white/30 hover:border-white/50 transition-all duration-300 hover:scale-105 hover:shadow-xl hover:shadow-white/20 focus:outline-none focus:ring-2 focus:ring-white/50"
                title="Rework Reports"
              >
                <span className="text-xs xl:text-sm font-semibold text-white">
                  Rework
            ...
            <div className="relative">
              <button
                ref={searchBtnRef}
                onClick={toggleSearchDropdown}
                className="glow-button group relative w-10 h-10 xl:w-14 xl:h-14 glass-effect hover:bg-white/30 rounded-xl sm:rounded-2xl border border-white/40 hover:border-white/60 transition-all duration-500 hover:scale-110 focus:outline-none focus:ring-2 sm:focus:ring-4 focus:ring-white/40 flex items-center justify-center overflow-hidden"
                title="Search"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
          ...
          window.matchMedia("(max-width: 1023px)").matches && (
            <>
              <div
                ...
                onClick={() => setIsReworkReportsOpen(false)}
              />
              <div className="fixed inset-2 top-16 sm:inset-4 sm:top-20 md:top-24 z-[100]">
                <ReworkSelectorModal
                  onClose={() => setIsReworkReportsOpen(false)}
                  onGenerate={handleReworkGenerate}
                  isMobile={true}
        ...
        {/* ---------- Report Modal (child window) ---------- */}
        {isReportModalOpen && (
          <div className="fixed inset-0 bg-black/50 z-[9999] flex items-center justify-center backdrop-blur-sm p-1 sm:p-2 md:p-4">
            <div className="bg-white dark:bg-gray-900 rounded-lg sm:rounded-xl md:rounded-2xl shadow-2xl w-full h-full sm:w-[98%] sm:h-[96%] md:w-[95%] md:h-[95%] lg:w-[90%] lg:h-[90%] overflow-hidden relative border border-purple-300 flex flex-col">
              {/* Top toolbar */}
              <div className="flex flex-col min-[400px]:flex-row items-start min-[400px]:items-center justify-between gap-1.5 sm:gap-2 md:gap-3 px-2 sm:px-3 md:px-4 py-1.5 sm:py-2 md:py-3 border-b border-gray-100 dark:border-gray-800 bg-white/60 dark:bg-gray-900/60">
                <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3 flex-wrap">
                  <strong className="text-[10px] sm:text-xs md:text-sm text-gray-700 dark:text-gray-200">
                    {reportType === "equipment"
                      ? "Equipment Pass Rate Report"
                      : reportType === "rework-approved"
                        ? "Rework Approved Report"
                ...
                <div className="flex items-center gap-1.5 sm:gap-2">
                  <button
                    ...
                    className="px-1.5 sm:px-2 md:px-3 py-0.5 sm:py-1 text-[10px] sm:text-xs md:text-sm rounded-md bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700"
                  >
                    <span className="hidden min-[400px]:inline">
                      Open in tab
                    </span>
                    <ExternalLink className="w-3 h-3 min-[400px]:hidden" />
                  </button>
                  ...
                  <button
                    ...
                    className="p-1 sm:p-1.5 md:p-2 rounded-full bg-gray-200 hover:bg-gray-300 dark:bg-gray-800 dark:hover:bg-gray-700"
                    title="Close"
                  >
                    <X className="w-3 h-3 sm:w-4 sm:h-4 text-gray-700 dark:text-gray-300" />
                  </button>
              ...
              <div className="flex-1 relative bg-gray-100 dark:bg-black overflow-hidden">
                {!iframeLoaded && (
                  <div className="absolute inset-0 flex items-center justify-center bg-white dark:bg-gray-900 z-10">
                    <div className="flex flex-col items-center justify-center px-4">
                      {/* ================= INDUSTRIAL LOADER - RESPONSIVE ================= */}
                      <div className="relative w-24 sm:w-32 md:w-36 lg:w-48 h-16 sm:h-20 md:h-24 lg:h-32">
                        {/* Pipeline */}
                        <div className="absolute top-1/2 left-0 right-0 -translate-y-1/2 h-1.5 sm:h-2 md:h-2.5 lg:h-3 bg-gradient-to-r from-slate-400 via-slate-500 to-slate-400 dark:from-slate-600 dark:via-slate-500 dark:to-slate-600 rounded-full shadow-inner">
                          <div className="absolute inset-0 flex items-center justify-around">
                            {[0, 1, 2, 3, 4].map((i) => (
                              <div
                                key={i}
                                className="w-0.5 sm:w-1 h-full bg-slate-300 dark:bg-slate-700"
                                style={{
                                  animation:
                                    "pulseSegment 1.5s ease-in-out infinite",
                                  animationDelay: `${i * 0.3}s`,
                        ...
                        <div className="absolute inset-0 flex items-center">
                          {[0, 1, 2, 3].map((i) => (
                            ...
                            >
                              <div className="relative">
                                <div className="w-4 sm:w-6 md:w-8 lg:w-10 h-4 sm:h-6 md:h-8 lg:h-10 bg-gradient-to-br from-cyan-400 to-blue-500 rounded-lg shadow-lg rotate-45">
                                  <div className="absolute inset-0.5 sm:inset-1 border sm:border-2 border-cyan-200 rounded-lg"></div>
                                </div>
                                <div className="absolute top-1/2 right-full w-3 sm:w-4 md:w-6 lg:w-8 h-0.5 bg-gradient-to-l from-cyan-400 to-transparent"></div>
                              </div>
                            </div>
                          ))}
                        </div>
                        ...
                        {/* Left Scanner */}
                        <div className="absolute top-0 left-1/4 -translate-x-1/2 flex flex-col items-center">
                          <div className="w-4 sm:w-5 md:w-6 lg:w-8 h-8 sm:h-10 md:h-12 lg:h-16 bg-gradient-to-b from-purple-500 to-purple-600 rounded-t-lg shadow-lg relative">
                            <div
                              className="absolute left-1/2 -translate-x-1/2 w-0.5 h-full bg-purple-300"
                              style={{
                                animation:
                            ...
                            ></div>
                            <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-2 sm:w-2.5 md:w-3 lg:w-4 h-0.5 sm:h-1 bg-purple-300 rounded-full animate-pulse"></div>
                          </div>
                        </div>

                        {/* Right Scanner */}
                        <div className="absolute top-0 right-1/4 translate-x-1/2 flex flex-col items-center">
                          <div className="w-4 sm:w-5 md:w-6 lg:w-8 h-8 sm:h-10 md:h-12 lg:h-16 bg-gradient-to-b from-emerald-500 to-emerald-600 rounded-t-lg shadow-lg relative">
                            <div
                              className="absolute top-1/2 left-0 right-0 h-0.5 bg-emerald-300"
                              style={{
                                animation:
                            ...
                            ></div>
                            <div
                              className="absolute bottom-0 left-1/2 -translate-x-1/2 w-2 sm:w-2.5 md:w-3 lg:w-4 h-0.5 sm:h-1 bg-emerald-300 rounded-full animate-pulse"
                              style={{ animationDelay: "0.3s" }}
                            ></div>
                          </div>
                        </div>
                        ...
                        <div className="absolute -bottom-4 sm:-bottom-6 md:-bottom-8 left-1/2 -translate-x-1/2 flex gap-2 sm:gap-3">
                          <div className="flex flex-col items-center gap-0.5 sm:gap-1">
                            <div className="w-1.5 sm:w-2 md:w-2.5 lg:w-3 h-1.5 sm:h-2 md:h-2.5 lg:h-3 bg-emerald-500 rounded-full shadow-md shadow-emerald-500 animate-pulse"></div>
                            <span className="text-[8px] sm:text-[9px] md:text-[10px] lg:text-xs text-gray-600 dark:text-gray-400">
                              Quality OK
                            </span>
                          </div>
                        </div>
                      ...
                      <div className="mt-6 sm:mt-8 md:mt-10 text-center">
                        <p className="text-xs sm:text-sm md:text-base lg:text-lg font-semibold text-gray-700 dark:text-gray-300 mb-0.5 sm:mb-1 md:mb-2">
                          Loading Report Data
                        </p>
                        <p className="text-[9px] sm:text-[10px] md:text-xs lg:text-sm text-gray-500 dark:text-gray-400">
                          Please wait while we fetch the traceability
                          information...
                        </p>
                      </div>
                ...
                <iframe
                  src={reportUrl}
                  title="Report"
                  className="w-full h-full border-none rounded-b-lg sm:rounded-b-xl md:rounded-b-2xl"
                  allowFullScreen
                  onLoad={() => setIframeLoaded(true)}
                  onError={() => {
                    setIframeLoaded(true);
        ...
        {/* ---------- Analytics 4.0 Child Window ---------- */}
        {isAnalyticsOpen && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-white/60 backdrop-blur-md p-1 sm:p-2 md:p-0">
            <div className="bg-white/80 backdrop-blur-xl rounded-lg sm:rounded-xl md:rounded-2xl shadow-2xl w-full h-full sm:w-[98%] sm:h-[96%] md:w-[98%] md:h-[95%] lg:w-[95%] lg:h-[90%] overflow-hidden relative border border-blue-400/40 flex flex-col">
              {/* Modal Header */}
              <div className="flex flex-col min-[400px]:flex-row items-start min-[400px]:items-center justify-between px-2 sm:px-3 md:px-5 py-1.5 sm:py-2 md:py-3 bg-gradient-to-r from-blue-50/90 via-purple-50/90 to-blue-50/90 backdrop-blur-xl border-b border-gray-200 gap-1.5 sm:gap-2">
                <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3">
                  <div className="p-1 sm:p-1.5 md:p-2 rounded-md sm:rounded-lg bg-blue-100">
                    <Factory className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 text-blue-600" />
                  </div>
                  <div>
                    <h2 className="text-gray-900 font-bold text-xs sm:text-sm md:text-base lg:text-lg">
                      Industry 4.0 Analytics Dashboard
                    </h2>
                ...
                <div className="flex items-center gap-1.5 sm:gap-2">
                  <button
                    ...
                    className="px-2 sm:px-3 md:px-4 py-1 sm:py-1.5 md:py-2 text-[10px] sm:text-xs md:text-sm rounded-md sm:rounded-lg md:rounded-xl bg-white/90 hover:bg-white text-gray-900 border border-gray-200/80 hover:border-blue-300 shadow-md sm:shadow-lg hover:shadow-xl transition-all duration-300 flex items-center gap-1 sm:gap-2 font-semibold"
                  >
                  ...
                  <button
                    onClick={() => setIsAnalyticsOpen(false)}
                    className="p-1 sm:p-1.5 md:p-2 rounded-lg sm:rounded-xl hover:bg-gray-200 transition-colors duration-200"
                    title="Close"
                  >
                    <X className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 text-gray-600 hover:text-gray-900" />
                  </button>
                </div>
              </div>
      ...
      {showUidToast && (
        <>
          ...
          <div className="fixed inset-0 z-[99999] flex items-center justify-center p-2 sm:p-4">
            <div className="bg-gradient-to-br from-orange-600/90 to-orange-400/90 backdrop-blur-lg border border-orange-500/60 rounded-lg sm:rounded-xl md:rounded-2xl lg:rounded-3xl shadow-2xl shadow-orange-700/30 w-full max-w-[280px] sm:max-w-sm md:max-w-md">
              <div className="flex items-center justify-between p-3 sm:p-4 md:p-6 border-b border-white/10">
                <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3">
                  <div className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 bg-white/20 rounded-md sm:rounded-lg md:rounded-xl flex items-center justify-center">
              ...
              <div className="p-3 sm:p-4 md:p-6 pt-0">
                <button
                  onClick={() => setShowUidToast(false)}
                  className="w-full bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white font-medium py-2 sm:py-2.5 md:py-3 px-3 sm:px-4 rounded-lg sm:rounded-xl transition-all duration-200 shadow-lg shadow-blue-500/25 text-xs sm:text-sm md:text-base"
                >
                  Understood
                </button>
              </div>
